...
void RunSimulation(string output_dir, int steps)
{
  ...
  // ===== Improved Euler =====
  {
    ...
    std::ofstream outfile(output_dir + "/circuit_improved_euler_" + steps_string + ".tsv");
  ...
  // ===== Implicit Euler =====
  {
    ...
    std::ofstream outfile(output_dir + "/circuit_implicit_euler_" + steps_string + ".tsv");
    outfile << 0.0 << "\t" << y(0) << "\t" << y(1) << std::endl;
  ...
  // ===== Crank-Nicolson =====
  {
    ...
    std::ofstream outfile(output_dir + "/circuit_crank_nicolson_" + steps_string + ".tsv");
    outfile << 0.0 << "  " << y(0) << " " << y(1) << std::endl;
...
int main(int argc, char *argv[])
{
  ...
  RunSimulation(output_dir, 80);
  RunSimulation(output_dir, 100);
